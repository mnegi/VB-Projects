...
Begin VB.Form frmAuthor 
   ...
   Begin VB.Timer Timer1 
      ...
      Top             =   15
   End
   Begin MSComctlLib.TreeView treePerson 
      Height          =   6075
      Left            =   150
      TabIndex        =   66
      TabStop         =   0   'False
...
End If
formactivated = True
'Set RS = db.OpenRecordset("select * from author", dbOpenDynamic, dbExecDirect, dbOptimistic)
End Sub
...
ImageList2.ImageHeight = 15
ImageList2.ImageWidth = 20
'add the image from FileList to imaglist
For j = 1 To File1.ListCount
ImageList2.ListImages.Add j, CStr(File1.List(j)), LoadPicture(App.Path & "\Common\Icons\Countries\" & File1.List(j - 1))
Next

'set the imagecombo's imagelist to imagelist2
imgCmbCountry.ImageList = ImageList2
Dim dname As String

'Add the text and images to imagecombo, use imagelist2
...
Private Sub imgCmbCountry_Change()
If imgCmbCountry.Text = "India" Then
If disp = False Then
...
Private Sub imgCmbCountry_Click()
If imgCmbCountry.Text = "India" Then
...
Sub DeleteRecord()
Cmd.ActiveConnection = Cn
Cmd.CommandText = "delete from author where author_id = " & txtDriverId.Text
Cmd.Execute
'MsgBox "deleted"
rs.Close
Call UpdateTreeView(cmbSort.Text, cmbSortOrder.Text)
...
MenuVisible ("0000")
Call CLEAR
treePerson.Enabled = False
End Sub
Sub SaveRecord()
treePerson.Enabled = True
...
CmdText = "insert into author values(" & txtDriverId.Text & ",'" & txtFname.Text & "','" & txtMname.Text & "','" & txtLname.Text & "','" & cmbGender.Text & "','" & mskDob.Text & "','" & txtAddress.Text & "','" & txtCity.Text & "','" & cmbDist.Text & "','" & cmbStates.Text & "','" & imgCmbCountry.Text & "','" & txtPin.Text & "','','" & photoDisPath & "')"
...
'********************************************
'TRYING IT WITH RECORDSET ITSELF
'rs.AddNew
'rs.Fields("author_id") = txtDriverId.Text
'rs.Fields("FNAME") = txtFname.Text
'rs.Fields("MNAME") = txtMname.Text
'rs.Fields("LNAME") = txtLname.Text
'rs.Fields("SEX") = cmbGender.Text
'rs.Fields("DOB") = mskDob.Text
'rs.Fields("ADDRESS") = txtAddress.Text
'rs.Fields("CITY") = txtCity.Text
'rs.Fields("DISTT") = cmbDist.Text
'rs.Fields("STATE") = cmbStates.Text
'rs.Fields("PIN") = txtPin.Text
'rs.Fields("PHONE") = txtPin.Text
'rs.Fields("PHOTO") = "Nothing"
'
'rs.Update
'rs.Close
'********************************************
...
Cmd.ActiveConnection = Cn
Cmd.CommandText = "select * from author where " & cmbSearch.Text & "='" & srchText & "'"
...
rsD.Source = "select * from author order by author_id"
...
treePerson.ImageList = imgListTree
treePerson.Nodes.CLEAR

Set treenode = treePerson.Nodes.Add(, , "authors", "author Database", "Close")
Cmd.ActiveConnection = Cn
Cmd.CommandText = "select * from AUTHOR order by " & a & " " & b
Set rs = Cmd.Execute

'rs.Open "select * from author order by author_id", Cn
...
While Not rs.EOF
    If rs.Fields("SEX") = "Male" Then
        Set treenode = treePerson.Nodes.Add("authors", tvwChild, "Key" & rs.Fields("author_id"), rs.Fields("author_id"), "Male")
    Else
        Set treenode = treePerson.Nodes.Add("authors", tvwChild, "Key" & rs.Fields("author_id"), rs.Fields("author_id"), "Female")
    End If
rs.MoveNext
...
disp = True
    txtDriverId.Text = DRS.Fields("author_id")
...
'    End If
    ...
    cmbStates.CLEAR
    If Not IsNull(DRS.Fields("STATE")) Then
    ...
    cmbDist.CLEAR
    If Not IsNull(DRS.Fields("STATE")) Then
        cmbDist.AddItem DRS.Fields("DISTT")
    End If
    ...
    If imgCmbCountry.ComboItems(i).Text = DRS.Fields("COUNTRY") Then
    ...
    Else
        'Call PhotoEnable("01")
        photoDisPath = DRS.Fields("photo")
        If ManiExtras1.FileExists(DRS.Fields("photo")) Then
        imgPhotoDisplay.Picture = LoadPicture(DRS.Fields("photo"))
        Else
        imgPhotoDisplay.Picture = Nothing
...
Private Sub treePerson_Click()
Call MenuVisible("0000")
fraShowAll.Visible = False
End Sub

Private Sub treePerson_Collapse(ByVal Node As MSComctlLib.Node)
Node.Image = "Close"
End Sub
'
'Private Sub treePerson_DblClick()
'End Sub

Private Sub treePerson_Expand(ByVal Node As MSComctlLib.Node)
Node.Image = "Open"
End Sub

Private Sub treePerson_NodeClick(ByVal Node As MSComctlLib.Node)

'***************************************
'TRYING TO UNCHECK ALL OTHER CHECKBOXES AS ONE IS CLICKED
'***************************************
'For i = 0 To treePerson.Nodes.Count - 1
'treePerson.Nodes(i).Checked = False
'Next
'Node.Checked = True
'
'***************************************
If Node.Text = "author Database" Then
Else
If Node.Checked = True Then
'rs.Close
Cmd.ActiveConnection = Cn
Cmd.CommandText = "select * from author where author_id = " & Node.Text
...
'Set rs1 = DB.OpenRecordset("select * from author where author_id = '" & txtDriverId.Text & "' ", dbOpenDynamic, dbExecDirect, dbOptimistic)
''rs1.Delete
'MsgBox "deleted"
'rs1.AddNew
'MsgBox rs1.RecordCount
'rs1.Fields("author_id") = txtDriverId.Text
...
Cmd.ActiveConnection = Cn
Cmd.CommandText = "update author set fname = '" & txtFname.Text & "',lname = '" & txtLname.Text & "',mname = '" & txtMname.Text & "',sex = '" & cmbGender.Text & "',dob = '" & mskDob.Text & "',distt = '" & cmbDist.Text & "',state = '" & cmbStates.Text & "',country='" & imgCmbCountry.Text & "',address = '" & txtAddress.Text & "',city = '" & txtCity.Text & "',pin = '" & txtPin.Text & "',photo = '" & photoDisPath & "' where author_id = " & txtDriverId.Text
Cmd.Execute
MsgBox "updated"
...
Cmd.ActiveConnection = Cn
Cmd.CommandText = "select * from author where " & cmbSearch.Text & " like '" & txtSearch.Text & "%'"
...
While Not rs1.EOF
    If rs1.Fields("SEX") = "Male" Then
        Set treenode = treePerson.Nodes.Add(, , rs1.Fields("author_id"), rs1.Fields("author_id"), "Male")
    Else
        Set treenode = treePerson.Nodes.Add(, , rs1.Fields("author_id"), rs1.Fields("author_id"), "Female")
    End If
rs1.MoveNext
Wend
...
Cmd.ActiveConnection = Cn
Cmd.CommandText = "select * from author where author_id=" & DId
