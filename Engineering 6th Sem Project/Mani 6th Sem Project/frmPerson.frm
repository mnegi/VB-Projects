...
End If
formactivated = True
'Set RS = db.OpenRecordset("select * from PERSON", dbOpenDynamic, dbExecDirect, dbOptimistic)
End Sub
...
Sub DeleteRecord()
Cmd.ActiveConnection = Cn
Cmd.CommandText = "delete from person where driver_id = '" & txtDriverId.Text & "'"
Cmd.Execute
'MsgBox "deleted"
rs.Close
Call UpdateTreeView(cmbSort.Text, cmbSortOrder.Text)
...
CmdText = "insert into person values('" & txtDriverId.Text & "','" & txtFname.Text & "','" & txtMname.Text & "','" & txtLname.Text & "','" & cmbGender.Text & "','" & mskDob.Text & "','" & txtAddress.Text & "','" & txtCity.Text & "','" & cmbDist.Text & "','" & cmbStates.Text & "','" & txtPin.Text & "','" & txtPhone.Text & "','" & CommonDialog1.FileTitle & "')"
...
'rs.AddNew
'rs.Fields("DRIVER_ID") = txtDriverId.Text
...
Cmd.ActiveConnection = Cn
Cmd.CommandText = "select * from person where " & cmbSearch.Text & "='" & srchText & "'"
...
Set treenode = treePerson.Nodes.Add(, , "Persons", "Person Database", "Close")
Cmd.ActiveConnection = Cn
Cmd.CommandText = "select * from person order by " & a & " " & b
Set rs = Cmd.Execute

'rs.Open "select * from person order by driver_id", Cn
...
While Not rs.EOF
    If rs.Fields("SEX") = "Male" Then
        Set treenode = treePerson.Nodes.Add("Persons", tvwChild, rs.Fields("DRIVER_ID"), rs.Fields("DRIVER_ID"), "Male")
    Else
        Set treenode = treePerson.Nodes.Add("Persons", tvwChild, rs.Fields("DRIVER_ID"), rs.Fields("DRIVER_ID"), "Female")
    End If
rs.MoveNext
...
disp = True
    txtDriverId.Text = DRS.Fields("DRIVER_ID")
    ...
    If DRS.Fields("photo") = "Nothing" Or IsNull(DRS.Fields("photo")) Then
        imgPhotoDisplay.Picture = Nothing
       ' lblImgLogo.Visible = True
    Else
        'Call PhotoEnable("01")
...
'
'***************************************
If Node.Text = "Person Database" Then
Else
If Node.Checked = True Then
'rs.Close
Cmd.ActiveConnection = Cn
Cmd.CommandText = "select * from person where driver_id = '" & Node.Text & "'"
...
'Set rs1 = DB.OpenRecordset("select * from person where driver_id = '" & txtDriverId.Text & "' ", dbOpenDynamic, dbExecDirect, dbOptimistic)
''rs1.Delete
'MsgBox "deleted"
'rs1.AddNew
'MsgBox rs1.RecordCount
'rs1.Fields("DRIVER_ID") = txtDriverId.Text
...
Cmd.ActiveConnection = Cn
Cmd.CommandText = "update person set fname = '" & txtFname.Text & "',lname = '" & txtLname.Text & "',mname = '" & txtMname.Text & "',sex = '" & cmbGender.Text & "',dob = '" & mskDob.Text & "',distt = '" & cmbDist.Text & "',state = '" & cmbStates.Text & "',address = '" & txtAddress.Text & "',city = '" & txtCity.Text & "',pin = '" & txtPin.Text & "',phone = '" & txtPhone.Text & "',photo = '" & CommonDialog1.FileTitle & "' where driver_id = '" & txtDriverId.Text & "'"
Cmd.Execute
MsgBox "updated"
...
Cmd.ActiveConnection = Cn
Cmd.CommandText = "select * from person where " & cmbSearch.Text & " like '" & txtSearch.Text & "%'"
...
While Not rs1.EOF
    If rs1.Fields("SEX") = "Male" Then
        Set treenode = treePerson.Nodes.Add(, , rs1.Fields("DRIVER_ID"), rs1.Fields("DRIVER_ID"), "Male")
    Else
        Set treenode = treePerson.Nodes.Add(, , rs1.Fields("DRIVER_ID"), rs1.Fields("DRIVER_ID"), "Female")
    End If
rs1.MoveNext
Wend
...
Cmd.ActiveConnection = Cn
Cmd.CommandText = "select * from person where driver_id='" & DId & "'"
